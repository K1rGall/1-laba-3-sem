...
#include <QTextEdit>
#include <chrono>
#include "testing\\smart_ptrs_tests.h"
#include "testing\\memory_checker.h"
#include "testing\\test_structure.h"
#include "QtWidgets"
#include "algorithm"
#include <QtCharts>
...
private slots:
    void createUnqPtrSlot(bool for_rewrite = false) {
        bool ok;
        int value = QInputDialog::getInt(this, "Input Value", "Enter an int value for UnqPtr:", 0, INT_MIN, INT_MAX, 1, &ok);

        if (ok) {
            uniq_ptr = UnqPtr<int>(new int(value));
            QString result = "Created UnqPtr with value: " + QString::number(*uniq_ptr);
            if(!for_rewrite) showResults("UnqPtr", result, 800, 600);
        }
    }
    ...
    void rewriteUnqPtrSlot() {
        ...
        showResults("UnqPtr", result, 800, 600);
    }

    void releaseUnqPtrSlot() {
        uniq_ptr.release();
        QString result = "UnqPtr released";
        showResults("UnqPtr", result, 800, 600);
    }
    ...
    void createShrdPtrSlot(bool for_rewrite = false) {
        ...
        if (ok) {
            ...
            if(!for_rewrite) showResults("ShrdPtr", result, 800, 600);
        }
    }
    ...
    void rewriteShrdPtrSlot() {
        ...
        showResults("ShrdPtr", result, 800, 600);
    }
    ...
    void showComparisonChartUniq() {
        ...
        for (size_t i = 0; i < std::min(UnqPtrTests.size(), static_cast<size_t>(6)); ++i) {
            unqPtrSeries->append(UnqPtrTests[i].second, UnqPtrTests[i].first);
            unqPtrScatterSeries->append(UnqPtrTests[i].second, UnqPtrTests[i].first);
        }

        for (size_t i = 0; i < std::min(StdUnqPtrTests.size(), static_cast<size_t>(6)); ++i) {
            stdUnqPtrSeries->append(StdUnqPtrTests[i].second, StdUnqPtrTests[i].first);
            stdUnqPtrScatterSeries->append(StdUnqPtrTests[i].second, StdUnqPtrTests[i].first);
        }
    ...
    void showComparisonChartShared() {
        ...
        for (size_t i = 0; i < std::min(ShrdPtrTests.size(), static_cast<size_t>(6)); ++i) {
            ShrdPtrSeries->append(ShrdPtrTests[i].second, ShrdPtrTests[i].first);
            ShrdPtrScatterSeries->append(ShrdPtrTests[i].second, ShrdPtrTests[i].first);
        }

        for (size_t i = 0; i < std::min(StdShrdPtrTests.size(), static_cast<size_t>(6)); ++i) {
            stdShrdPtrSeries->append(StdShrdPtrTests[i].second, StdShrdPtrTests[i].first);
            stdShrdPtrScatterSeries->append(StdShrdPtrTests[i].second, StdShrdPtrTests[i].first);
        }
